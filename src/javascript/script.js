document.querySelector('#search').addEventListener('submit', async (event) =>{
    ...
    const resuts = await fetch(apiUrl);
    const json = await resuts.json();
    ...
    if(json.cod === 200){
        ...
        document.querySelector('#weather').classList.remove('show');
         document.querySelector('#container').classList.remove('expandido');
        showAlert(`
            Não conseguimos encontrar a cidade...
